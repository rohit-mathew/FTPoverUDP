 ...
 */
#define	RTT_RTOCALC(ptr) ((ptr)->rtt_srtt + ((ptr)->rtt_rttvar)>>3)
...
rtt_ts(struct rtt_info *ptr)
{
	...
	Gettimeofday(&tv, NULL);
	//ts = ((tv.tv_usec - ptr->rtt_base) / 1000) + (tv.tv_usec / 1000);
...
rtt_newpack(struct rtt_info *ptr)
{
	//printf("\nIn new pack\n");
	ptr->rtt_nrexmt = 0;
	//printf("The ptr->rtt_nrexmt is: %d",ptr->rtt_nrexmt);
}

int
rtt_start(struct rtt_info *ptr)
{
	return(ptr->rtt_rto/1000);		/* round float to int */
...
rtt_stop(struct rtt_info *ptr, uint32_t ms)
{
	...
	ptr->rtt_srtt += (int) delta >> 3;		/* g = 1/8 */

	if (delta < 0)
		delta = -delta;				/* |delta| */

	ptr->rtt_rttvar += (int) (delta - ptr->rtt_rttvar) >> 2;	/* h = 1/4 */
...
{	
	...
	ptr->rtt_rto = rtt_minmax(ptr->rtt_rto); /* Restrict the max RTO */
